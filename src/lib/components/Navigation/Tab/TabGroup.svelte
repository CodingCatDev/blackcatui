<script lang="ts">
	// Slots
	/**
	 * @slot bcu-tab-group-panel - An optional region to house tab content.
	 */
...
<div
	class="bcu-tab-group {classesBase}"
	data-testid="bcu-tab-group"
	on:click
	on:keypress
	on:keydown
	on:keyup
>
	<!-- Tab List -->
	<div class="bcu-tab-group-list {classesList}" role="tablist" aria-labelledby={labelledby}>
		<slot />
	</div>
	<!-- Tab Panel -->
	{#if $$slots['bcu-tab-group-panel']}
		<div class="bcu-tab-group-panel {classesPanel}" role="tabpanel" aria-labelledby={panel}>
			<slot name="bcu-tab-group-panel" />
		</div>
	{/if}
</div>
