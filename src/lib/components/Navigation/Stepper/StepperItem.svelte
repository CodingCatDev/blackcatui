...
{#if stepIndex === $state.current}
	<div class="bcu-stepper-item {classesBase}" data-testid="bcu-stepper-item">
		<!-- Slot: Header -->
		<header class="bcu-stepper-item-header {classesHeader}">
			<slot name="bcu-stepper-item-header">{stepTerm} {stepIndex + 1}</slot>
		</header>
		<!-- Slot: Default -->
		<div class="bcu-stepper-item-content {classesContent}">
			<slot>({stepTerm} {stepIndex + 1} Content)</slot>
		</div>
		<!-- Navigation -->
		{#if $state.total > 1}
			<div
				class="bcu-stepper-item-navigation {classesNavigation}"
				transition:fade|local={{ duration: 100 }}
			>
				<button
					type={buttonBackType}
					class="bcu-button {buttonBack}"
					on:click={onBack}
					disabled={$state.current === 0}>{@html buttonBackLabel}</button
				>
				{#if stepIndex < $state.total - 1}
					<button
						type={buttonNextType}
						class="bcu-button {buttonNext}"
						on:click={onNext}
						disabled={locked}
					>
						{#if locked}
				...
				{:else}
					<button
						...
						class="bcu-button {buttonComplete}"
						on:click={onComplete}
						disabled={locked}
					>
