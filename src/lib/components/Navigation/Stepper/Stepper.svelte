...
<div class="bcu-stepper {classesBase}" data-testid="bcu-stepper">
	<!-- Header -->
	{#if $state.total}
		<header class="bcu-stepper-header {classesHeader}" transition:fade|local={{ duration: 100 }}>
			{#each Array.from(Array($state.total).keys()) as step}
				<div class="bcu-stepper-header-step {classesHeaderStep}" class:flex-1={isActive(step)}>
					<span class="bcu-badge {classesBadge(step)}"
						>{isActive(step) ? `${stepTerm} ${step + 1}` : step + 1}</span
					>
				</div>
			{/each}
		</header>
	{/if}
	<!-- Content -->
	<div class="bcu-stepper-content {classesContent}">
		<slot />
	</div>
</div>
