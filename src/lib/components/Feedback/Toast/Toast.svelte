<script lang="ts">
	...
	export let buttonAction: CssClasses = 'bcu-button variant-filled';
	/** Provide styles for the dismiss button. */
	export let buttonDismiss: CssClasses = 'bcu-button-icon bcu-button-icon-sm variant-filled';
...
{#if $toastStore.length}
	<!-- Wrapper -->
	<div class="bcu-snackbar-wrapper {classesWrapper}" data-testid="bcu-snackbar-wrapper">
		<!-- List -->
		<div
			class="bcu-snackbar {classesSnackbar}"
		...
		>
			{#each filteredToasts as t, i (t)}
				<div animate:flip={{ duration }}>
					<!-- Toast -->
					<div
						class="bcu-toast {classesToast} {t.background ?? background} {t.classes}"
						role="alert"
						aria-live="polite"
						data-testid="toast"
					>
						<div class="bcu-toast-text-base">{@html t.message}</div>
						<div class="bcu-toast-toast-actions {cToastActions}">
