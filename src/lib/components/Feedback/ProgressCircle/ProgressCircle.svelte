...
<script lang="ts">
	...
	// Base Classes
	const cBase = 'bcu-progress-circle relative overflow-hidden';
...
<figure
	class="bcu-progress-circle {classesBase}"
	data-testid="bcu-progress-circle"
	role="meter"
...
>
	...
	>
		<!-- Track -->
		<circle
			class="bcu-progress-circle-track {cBaseTrack} {track}"
			stroke-width={stroke}
			r={baseSize / 2}
			cx="50%"
			cy="50%"
		...
		<!-- Meter -->
		<circle
			class="bcu-progress-circle-meter {cBaseMeter} {meter}"
			stroke-width={stroke}
			r={baseSize / 2}
			cx="50%"
			cy="50%"
		...
		{#if value != undefined && value >= 0 && $$slots.default}
			<text
				...
				font-weight="bold"
				font-size={font}
				class="bcu-progress-circle-text {fill}"><slot /></text
			>
		{/if}
	</svg>
</figure>
