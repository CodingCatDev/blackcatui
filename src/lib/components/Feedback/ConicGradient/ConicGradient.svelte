...
<figure class="bcu-conic-gradient {classesBase}" data-testid="bcu-conic-gradient">
	<!-- Label -->
	{#if $$slots.default}
		<figcaption class="bcu-conic-caption {classesCaption}"><slot /></figcaption>
	{/if}
	<!-- Conic Gradient -->
	{#if cone}
		<div class="bcu-conic-cone {classesCone}" class:animate-spin={spin} style:background={cone} />
	{/if}
	<!-- Legend -->
	{#if legend && generatedLegendList}
		<ul class="bcu-conic-list bcu-list {classesLegend}">
			{#each generatedLegendList as { color, label, value }}
				<li class="bcu-conic-item {hover}" on:click on:keydown on:keyup on:keypress>
					<span class="bcu-conic-swatch {cSwatch}" style:background={color} />
					<span class="bcu-conic-label flex-auto">{label}</span>
					<strong class="bcu-conic-value">{value}%</strong>
				</li>
			{/each}
		</ul>
	{/if}
