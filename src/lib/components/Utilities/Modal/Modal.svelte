<script lang="ts">
	...
	function onBackdropInteraction(event: MouseEvent | TouchEvent): void {
		...
		if (event.target.classList.contains('bcu-modal-backdrop')) onClose();
		if (event.target.classList.contains('bcu-modal-transition')) onClose();
...
{#if $modalStore.length > 0}
	{#key $modalStore}
		<!-- Backdrop -->
		<div
			class="bcu-modal-backdrop {classesBackdrop}"
			data-testid="bcu-modal-backdrop"
		...
		>
			<!-- Transition Layer -->
			<div
				class="bcu-modal-transition {classesTransitionLayer}"
				transition:fly={{ duration, opacity: flyOpacity, x: flyX, y: flyY }}
			>
				{#if $modalStore[0].type !== 'component'}
					<!-- Modal: Presets -->
					<div
						class="bcu-modal {classesModal}"
						data-testid="bcu-modal"
						role="dialog"
						aria-modal="true"
					...
					>
						...
						{#if $modalStore[0]?.title}
							<header class="bcu-modal-header {regionHeader}">{@html $modalStore[0].title}</header>
						{/if}
						<!-- Body -->
						{#if $modalStore[0]?.body}
							<article class="bcu-modal-body {regionBody}">{@html $modalStore[0].body}</article>
						{/if}
						<!-- Image -->
						{#if $modalStore[0]?.image && typeof $modalStore[0]?.image === 'string'}
							<img class="bcu-modal-image {cModalImage}" src={$modalStore[0]?.image} alt="Modal" />
						{/if}
						<!-- Type -->
						{#if $modalStore[0].type === 'alert'}
							<!-- Template: Alert -->
							<footer class="bcu-modal-footer {regionFooter}">
								<button type="button" class="bcu-button {buttonNeutral}" on:click={onClose}
						...
						{:else if $modalStore[0].type === 'confirm'}
							...
							<footer class="bcu-modal-footer {regionFooter}">
								<button type="button" class="bcu-button {buttonNeutral}" on:click={onClose}
									>{buttonTextCancel}</button
								>
								<button type="button" class="bcu-button {buttonPositive}" on:click={onConfirm}
						...
						{:else if $modalStore[0].type === 'prompt'}
							...
							<form class="space-y-4" on:submit={onPromptSubmit}>
								<input
									class="bcu-modal-prompt-input input"
									name="prompt"
									type="text"
								...
								<footer class="bcu-modal-footer {regionFooter}">
									<button type="button" class="bcu-button {buttonNeutral}" on:click={onClose}
										>{buttonTextCancel}</button
									>
									<button type="submit" class="bcu-button {buttonPositive}"
				...
				{:else}
					...
					<div
						class="bcu-modal contents {$modalStore[0].modalClasses ?? ''}"
						data-testid="bcu-modal-component"
						role="dialog"
						aria-modal="true"
						aria-label={$modalStore[0].title ?? ''}
					>
