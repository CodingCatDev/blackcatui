...
import type { ModalSettings } from '$lib/components/Utilities/Modal/types';
...
describe('Modal.svelte', () => {
	it('Renders modal alert', async () => {
		...
		expect(getByTestId('bcu-modal-backdrop')).toBeTruthy();
		expect(getByTestId('bcu-modal')).toBeTruthy();
	});
