<script lang="ts">
	...
	export let slotLead: CssClasses = 'mb-4 flex justify-center items-center';
...
<div
	class="bcu-file-dropzone {classesBase}"
	class:opacity-50={$$restProps.disabled}
	data-testid="bcu-file-dropzone"
>
	<!-- Input: File (hidden) -->
	...
	<input
		...
		type="file"
		{name}
		class="bcu-file-dropzone-input {classesInput}"
		{...prunedRestProps()}
		on:change
		on:dragenter
		on:dragover
	...
	/>
	<!-- Interface -->
	<div class="bcu-file-dropzone-interface {classesInterface} {regionInterface}">
		<div class="bcu-file-dropzone-interface-text {regionInterfaceText}">
			<!-- Lead -->
			{#if $$slots.lead}<div class="bcu-file-dropzone-lead {slotLead}">
					<slot name="lead" />
				</div>{/if}
			<!-- Message -->
			<div class="bcu-file-dropzone-message {slotMessage}">
				<slot name="bcu-message"><strong>Upload a file</strong> or drag and drop</slot>
			</div>
			<!-- Meta Text -->
			{#if $$slots['bcu-file-dropzone-meta']}<small class="bcu-file-dropzone-meta {slotMeta}"
					><slot name="bcu-file-dropzone-meta" /></small
				>{/if}
		</div>
	</div>
</div>
