<script lang="ts">
	...
	export let regionList: string = 'bcu-native-list-nav';
	...
	function whitelistOptions(): void {
		...
		listedOptions = [...options].filter((option: AutocompleteOption) =>
			whitelist.includes(option.value)
		);
	}
	...
	function blacklistOptions(): void {
		...
		listedOptions = [...options].filter(
			(option: AutocompleteOption) => !toBlacklist.has(option.value)
		);
	}
...
<div class="bcu-autocomplete {classsesBase}" data-testid="bcu-autocomplete">
	{#if optionsFiltered.length > 0}
		<nav class="bcu-autocomplete-nav {classesNav}">
			<ul class="bcu-autocomplete-list {classesList}">
				{#each optionsFiltered as option, i (option)}
					<li
						class="bcu-autocomplete-item {classesItem}"
						animate:flip={{ duration }}
						transition:slide|local={{ duration }}
					>
						<button
							class="bcu-autocomplete-button {classesButton}"
							type="button"
	...
	{:else}
		<div class="bcu-autocomplete-empty {classesEmtpy}">{emptyState}</div>
	{/if}
</div>
