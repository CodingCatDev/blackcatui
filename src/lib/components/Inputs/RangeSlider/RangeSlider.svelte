...
<div class="bcu-range-slider {classesBase}" data-testid="bcu-range-slider">
	<!-- Slot: Default -->
	{#if $$slots.default}<label class="bcu-range-slider-label {cBaseLabel}" for={id}><slot /></label
		>{/if}
	...
	<div class="range-content {cBaseContent}">
		...
		<input
			...
			{id}
			{name}
			class="bcu-range-slider-input {classesInput}"
			list="tickmarks-{id}"
			aria-label={label}
			{min}
			{max}
		...
		{#if ticked && tickmarks && tickmarks.length}
			<datalist id="tickmarks-{id}" class="bcu-range-slider-ticks">
	...
	<!-- Slot: Trail -->
	{#if $$slots.trail}<div class="bcu-range-slider-trail"><slot name="trail" /></div>{/if}
</div>
