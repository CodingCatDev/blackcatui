<script lang="ts">
	...
	// Base Classes
	const cBase =
		'unstyled grid place-content-center place-items-center w-full aspect-square space-y-1.5 cursor-pointer';
...
<div on:click={onClickHandler} on:click on:keydown on:keyup on:keypress>
	...
	<svelte:element this={tag} {...prunedRestProps()} class="bcu-app-rail-tile {classesBase}">
		<!-- Slot: Default (icon) -->
		{#if $$slots.default}
			<div class="bcu-app-rail-tile-icon {regionIcon}"><slot /></div>
		{/if}
		<!-- Label -->
		{#if label}
			<div class="bcu-app-rail-tile-label {classesLabel}">{label}</div>
		{/if}
	</svelte:element>
</div>
