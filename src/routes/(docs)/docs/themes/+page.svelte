<script lang="ts">
	...
	import { ExclamationTriangle, ArrowTopRightOnSquare, ArrowDownTray } from '@steeze-ui/heroicons';
...
<DocsLayout>
	<svelte:fragment slot="bcu-docs-header">
		<div class="space-y-4">
			<h1>Themes</h1>
			<p>Learn more about customizing BlackCatUI themes.</p>
		</div>
	</svelte:fragment>
	<svelte:fragment slot="bcu-docs-detail">
		<section class="flex flex-col gap-10">
			<section class="sticky top-4 z-[1] bcu-card variant-glass p-4 space-y-4">
			...
			{#if activeTheme.file !== 'custom'}
				<section class="space-y-4">
					<h2>Preset Extras</h2>
					<p>
						When using preset themes provided by Skeleton, consider implementing the <code
							>data-theme</code
						>
						attribute on the <em>body</em>
						tag in <code>app.html</code>. This implements additional settings such as background
						gradients, header font weights, and more.
					</p>
			...
			<section class="space-y-4">
				<h2>Backgrounds</h2>
				<p>
					Your app's background is automatically set via a <a
						href="https://www.skeleton.dev/docs/tokens">design token</a
				...
				<div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-center">
					<div class="space-y-4">
						...
						<p>
							You may optionally provide a background image, including the use of CSS mesh
							gradients. Mix in theme color properties to create fully adaptive gradient
							backgrounds.
						</p>
					</div>
					<div>
						<a
							class="bcu-button variant-filled-secondary font-bold"
							href="https://csshero.org/mesher/"
							target="_blank"
							rel="noreferrer"
						>
							<span>Create a Mesh</span>
							<Icon src={ArrowTopRightOnSquare} class="w-6" />
						</a>
					</div>
				</div>
				<CodeBlock
	...
	body {
		...
		:root {
			...
			--theme-rounded-container: 4px;
				...
				/>
				<p>
					Likewise, if you wish to implement a custom the font family for a preset theme, you can
					modify the <em>base</em> and
					<em>heading</em>
			...
			<section class="space-y-4">
				...
				{#if activeTheme.fonts.length && activeTheme.file !== 'custom'}
					<aside class="bcu-alert variant-ghost-warning">
						<Icon src={ExclamationTriangle} class="w-6" />
						<div class="bcu-alert-message">
							<p>
								The <code>{activeTheme.name}</code> theme makes use of custom fonts. We recommend you
								follow the instruction below.
							</p>
						</div>
					</aside>
				{:else if activeTheme.file !== 'custom'}
					<aside class="bcu-alert variant-ghost">
						<div class="bcu-alert-message">
				...
				<TabGroup regionPanel="space-y-4">
					...
					<svelte:fragment slot="bcu-tab-group-panel">
						{#if tabsFontImport === 0}
							...
							<div class="bcu-card variant-glass p-4 flex justify-center items-center gap-2">
								{#each activeFonts as f}
									<a
										class="bcu-button variant-filled-primary"
										href={f.url}
										target="_blank"
										rel="noreferrer"
									>
										<Icon src={ArrowDownTray} class="w-6" />
										<span>{f.name}</span>
									</a>
								{/each}
							</div>
							...
							<p>
								...
								>
								directory in the root of your SvelteKit project. When available we recommend using variable
								fonts as they require only a single file. Otherwise copy all static font file assets
								to the <code>/static/fonts</code> directory.
							</p>
							{#each activeFonts as f}
		...
		@font-face {
			...
			src: url('/fonts/${f.file}');
							...
							<p>
								...
								>
								and
								<em>heading</em> properties. Be sure to use the same reference name set above or
								your font <u>will not work</u>.
							</p>
							{#each activeFonts as f}
								<CodeBlock
									language="css"
		...
		:root {
			...
			/* ... */
						...
						{:else if tabsFontImport === 1}
							<aside class="bcu-alert bcu-alert-message variant-ghost-warning">
								<p>
									<strong>Warning:</strong> please be aware that using remote imports are typically not
									GDPR compliant.
								</p>
							</aside>
							<!-- 1 -->
							...
							<p>
								...
								on the Web" section, choose <code>@import</code>, then copy the import statement.
							</p>
							<div class="bcu-card variant-glass p-4 flex justify-center items-center gap-2">
								{#each activeFonts as f}
									<a
