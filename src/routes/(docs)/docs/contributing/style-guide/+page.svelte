	...
	<svelte:fragment slot="bcu-docs-detail">
		<section class="flex flex-col gap-10">
			...
			<header class="space-y-4">
				<h1>Style Guide</h1>
				<p>
					If you wish to contribute to BlackCatUI, please review our opinionated code style guide
					below.
				</p>
			</header>
			...
			<section class="space-y-4">
				...
				<ul class="list-disc list-outside ml-4 space-y-1">
					<li>
						Feature directories should be singular and title case: <code>../LightSwitch/..</code>
					</li>
					<li>Components should be singular and title case: <code>LightSwitch.svelte</code></li>
					<li>
						Svelte Actions should be singular, lowercase, and use Typescript: <code
							>clipboard.ts</code
						>
					</li>
					<li>
						Tailwind Element stylesheets should be plural and lowercase: <code>buttons.css</code>
					</li>
					<li>
						Documentation should be lowercase and use dashes: <code
							>/routes/docs/design-tokens/+page.svelte</code
						>
					</li>
					<li>
						Tests should be suffixed with <code>*.test.ts</code> and match the feature naming
						convention: <code>LightSwitch.test.ts</code>
					</li>
				</ul>
			</section>
			...
			<section class="space-y-4">
				...
				<CodeBlock language="html" code={`<button on:click on:mouseover>BlackCatUI</button>`} />
				<p>
					Follow conventions set by existing components when naming slots. These should be short,
					semantic, and agnostic. All slots should be prefixed with <code>bcu-</code>.
				</p>
				<CodeBlock
					language="html"
					code={`{#if $$slots.bcu-code-block-lead}<slot name="bcu-code-block-lead" />{/if}`}
				/>
				<p>
					Use caution when inlining Tailwind color classes that would clash with custom themes.
					Ideally use a custom component property to allow for customization.
				</p>
				<CodeBlock
					language="html"
					code={`
❌ <div class="bg-orange-500">BlackCatUI</div>
✅ <div class="bg-secondary-500">BlackCatUI</div> 
		`.trim()}
				/>
				<!-- prettier-ignore -->
				<p>
...
function prunedRestProps(): any {
	...
	return $$restProps;
				...
				<CodeBlock
					language="html"
					code={`<button class="... {$$props.class ?? ''}" {...prunedRestProps()}>BlackCatUI</button>`}
				/>
			</section>
			...
			<section class="space-y-4">
				...
				<ul class="list-disc list-outside ml-4 space-y-1">
					<li>
						Each prop should be a single word, all lowercase, and semantic. Match Tailwind class
						naming conventions whenever possible.
					</li>
					<li>
						Color props should follow standard CSS style conventions (ex: <code>color</code> for text
						color).
					</li>
					<li>If you need multiple words, use camel-casing (ex: <code>ringWidth</code>).</li>
					<li>
						Never pass class props as arrays or objects. Strings work better (ex: <code
							>border border-primary-500</code
						>).
					</li>
					<li>
						Always pass the full Tailwind class name. <u
							>Tailwind does not support contructed class names</u
						>.
					</li>
					<li>
					...
					<li>
						If a new prop is added or modified then please consider updating the documentation with
...
export let visible: boolean = false;
		`}
			...
			<section class="space-y-4">
				...
				<p>
					BlackCatUI utilizes a specific paradigm for handling static and dynamic Tailwind styles
					for components. This is accomplished by passing Tailwind classes to the component as
					props, as well as by defining base structural classes within the component's script tag.
					While this may feel odd at first, you will find it works really well in execution.
				</p>
				<!-- Base Classes -->
				<h3>Base Classes</h3>
				<p>
					Any core or structural Tailwind classes can be defined as follows. Note the "c" is short
					for <strong>classes</strong>.
				</p>
				<CodeBlock
					language="ts"
					code={`
...
let cLabel: string = 'text-base'; // child element label styles
		`}
				...
				<p>
					If you expect to set one or more styles based on the current value of a property, handle
					this within a reactive statement as shown below.
				</p>
				<CodeBlock
					language="ts"
					code={`
...
$: classesLabel = \`\${cBaseLabel}\`; // child element
		`}
				...
				<ol class="list-decimal list-outside ml-4 space-y-1">
					<li>
						The first class should be an "identifier" class, providing a selection target for global
					...
					<li>
						For parent elements, add <code>$$props.classes</code> to supply additional arbitrary classes.
					</li>
				</ol>
				<CodeBlock
					language="html"
	...
	<span class="tab-label {classesLabel}">Label</span>
		...
		`}
			...
			<section class="space-y-4">
				<h2>Pitfalls</h2>
				<p>
					Below are a few pitfalls we've encountered when creating BlackCatUI. Do your best to avoid
			...
			<li>Keep BlackCatUI icon library agnostic. Embed SVGs or unicode instead. Implement a slot with a default so that the user can override if necessary.</li>
		</ul>
			</section>
		</section>
	</svelte:fragment>
