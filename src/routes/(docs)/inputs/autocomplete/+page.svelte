<script lang="ts">
	...
	// Types
	import type { AutocompleteOption } from '$lib/components/Inputs/Autocomplete/types';
	// Components
	import Autocomplete from '$lib/components/Inputs/Autocomplete/Autocomplete.svelte';
	...
	// Sveld
	import sveldAutocomplete from '$lib/components/Inputs/Autocomplete/Autocomplete.svelte?raw&sveld';
	...
	const settings: DocsShellSettings = {
		...
		source: 'components/Inputs/Autocomplete',
	...
	<section class="flex flex-col gap-4">
		<DocsPreview>
			<svelte:fragment slot="bcu-previewer-preview">
				<div class="text-token w-full max-w-sm space-y-2">
					...
					<Autocomplete
						...
						class="bcu-card w-full max-w-sm max-h-48 p-4 overflow-y-auto"
			...
			<svelte:fragment slot="bcu-previewer-source">
				...
				<p>
					Provide an array of objects containing <code>label</code> and <code>value</code> keys.
				</p>
				<CodeBlock
					language="ts"
					code={`
...
function onDemoSelection(event: any): void {
	inputDemo = event.detail.label;
				...
				<CodeBlock
					language="html"
					code={`<Autocomplete bind:input={inputDemo} options={demoOptions} on:selection={onDemoSelection} />`}
				/>
			</svelte:fragment>
		</DocsPreview>
		<!-- prettier-ignore -->
		...
		<section class="space-y-4">
			...
			<p>
				You may optionally append <code>keywords</code> to provide additional search terms. As well
				as <code>meta</code> to provide arbitrary data - which is not utilizing for filtering. All
				data option data is returned by <code>on:selection</code>, including these.
			</p>
			<CodeBlock
				language="ts"
				code={`
const flavorOptions: AutocompleteOption[] = [
	...
	{ ..., meta: { healthy: false } },
		...
		<section class="space-y-4">
			<h2>Whitelist</h2>
			<p>
				Provide a list of values you wish to whitelist. Only options with a matching value will be
				displayed.
			</p>
			<DocsPreview background="neutral" regionFooter="text-center">
				...
				<svelte:fragment slot="bcu-previewer-source">
					<CodeBlock
						language="ts"
						code={`const flavorWhitelist: string[] = ['neapolitan', 'pineapple', 'peach'];`}
					/>
		...
		<section class="space-y-4">
			<h2>Blacklist</h2>
			<p>
				Provide a list of values you wish to blacklist. Blacklisted options will be excluded from
				the list.
			</p>
			<DocsPreview background="neutral" regionFooter="text-center">
				<svelte:fragment slot="bcu-previewer-preview">
				...
				<svelte:fragment slot="bcu-previewer-source">
					<CodeBlock
						language="ts"
						code={`let flavorBlacklist: string[] = ['vanilla', 'chocolate'];`}
					/>
		...
		<section class="space-y-4">
			<h2>Input Chip</h2>
			<p>
				We've provide a demo of using Autocomplete alongside a BlackCatUI's Input Chip component
				below.
			</p>
			<DocsPreview background="neutral" regionFooter="text-center">
				<svelte:fragment slot="bcu-previewer-preview">
