<script lang="ts">
	...
	import PanelProps from './partials/PanelProps.svelte';
	...
	// Utilities
	import { docShellDefaults } from './defaults';
...
<DocsLayout class="bcu-doc-shell {classesBase}" tocKey={tabPanel}>
	...
	<svelte:fragment slot="bcu-docs-detail">
		...
		{#if tabs}
			<TabGroup class={classesTabs}>
				...
				{#if sveldCounts.props > 0}<Tab bind:group={tabPanel} name="properties" value="properties"
						>Props</Tab
					>{/if}
				{#if pageData?.parameters?.length}<Tab
						bind:group={tabPanel}
						name="parameters"
						value="parameters">Params</Tab
					>{/if}
				{#if sveldCounts.slots > 0}<Tab bind:group={tabPanel} name="slots" value="slots">Slots</Tab
					>{/if}
				{#if pageData?.classes?.length}<Tab bind:group={tabPanel} name="classes" value="classes"
						>Classes</Tab
					>{/if}
				{#if sveldCounts.events > 0}<Tab bind:group={tabPanel} name="events" value="events"
						>Events</Tab
					>{/if}
				{#if pageData?.keyboard?.length}<Tab bind:group={tabPanel} name="keyboard" value="keyboard"
						>Keyboard</Tab
					>{/if}
			</TabGroup>
		{/if}
