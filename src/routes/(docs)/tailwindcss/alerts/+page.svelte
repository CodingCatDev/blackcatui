<script lang="ts">
	...
	import { Sun, Moon, ExclamationTriangle, XMark } from '@steeze-ui/heroicons';
	...
	<section class="flex flex-col gap-10">
		<DocsPreview>
			<svelte:fragment slot="bcu-previewer-preview">
				<!-- Primary -->
				<aside class="bcu-alert {currentVariant}">
					<Icon src={ExclamationTriangle} class="w-16" />
					<div class="bcu-alert-message" data-toc-ignore>
					...
					<div class="bcu-alert-actions">
						...
						<button class="bcu-button-icon variant-filled">
							<Icon src={XMark} class="w-8" />
						</button>
					</div>
				</aside>
			</svelte:fragment>
...
{/if}
		...
		<p>
			Create an element with the <code>.alert</code> class. Wrap the alert in a Svelte
			<code>#if</code> statement to hide it or make it visible.
		</p>
		<section class="space-y-4">
			<h2>Sections</h2>
			<h3>Message Content</h3>
			<p>
				Use the <code>.alert-message</code> to create a vertical set of text information that fills the
				available width of the alert.
			</p>
			<CodeBlock
				language="html"
			...
			<p>
				Use the <code>.alert-actions</code> to create a trailing area to house interactive action buttons.
			</p>
		...
		<section class="space-y-4">
			...
			<CodeBlock
				language="html"
				code={`<aside class="alert" transition:fade|local={{ duration: 200 }}>(content)</div>`}
			/>
		</section>
	</section>
</DocsShell>
